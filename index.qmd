...
**Me: *Interactive mode*** is essentially running the codes on a line by line basis, it allows us to see which command is doing exactly what. We do this through the console. ***Script mode***, which was the 'source script' in R, runs the code all at once, which we did for the madlibs.py program. **Command-line mode** means we are running the code through the terminal.
...
I used the ggrepel package to make the labels all visible on the plot.

**Interpretation:**

A trend can be seen from the plot, where the earlier a character was hired the greater is the salary for that character. Example, Michael Scott was hired back in 1996 thus he has the highest salary whereas Pam Beesly was hired in 2002 and has the lowest salary among the four characters.
